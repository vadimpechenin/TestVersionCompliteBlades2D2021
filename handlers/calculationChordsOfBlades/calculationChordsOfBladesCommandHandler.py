...
from .allCalculationChordsOfBladesParameter import AllCalculationChordsOfBladesParameter

import numpy as np
import copy

class CalculationChordsOfBladesCommandHandler(BaseCommandHandler):
    def __init__(self):
        pass

    def execute(self, parameters):
        # Вычисления зазоров и натягов по порядку
        ...
        P_T = PointsForSquare(parameters.pointsBackThroughParams.pointsTroughParams.Point_1_T,
                              parameters.pointsBackThroughParams.pointsTroughParams.Point_2_T,
                              parameters.pointsBackThroughParams.pointsTroughParams.Point_3_T,
                              parameters.pointsBackThroughParams.pointsTroughParams.Point_4_T)
        calcSquare = CalculationNominalscommandHandler()
        gap_Squre_B = np.zeros(parameters.arrayNumberOfBlades.shape[0])
        gap_Squre_T = np.zeros(parameters.arrayNumberOfBlades.shape[0])
        delta_chord_T_B_array = np.zeros((parameters.arrayNumberOfBlades.shape[0], 2))
        for i in range(parameters.arrayNumberOfBlades.shape[0]):
            gap_Squre_B[i] = self.model_blade_error_2D_B(thickness_B_array[i], parameters.delta_angle[i],
                                                         parameters.angle,
                                                         parameters.pointsBackThroughParams.pointsBackParams.k0B,
                                                         parameters.pointsBackThroughParams.pointsBackParams.b0B, P_B,
                                                         calcSquare)
            gap_Squre_T[i] = self.model_blade_error_2D_T(thickness_T_array[i], parameters.delta_angle[i],
                                                         parameters.angle,
                                                         parameters.pointsBackThroughParams.pointsTroughParams.k0T,
                                                         parameters.pointsBackThroughParams.pointsTroughParams.b0T, P_T,
                                                         calcSquare)

            delta_chord_T_B_array[i, :] = self.gap_function_1(gap_Squre_T[i], gap_Squre_B[i],
                                                              parameters.pointsBackThroughParams.gap_Squre_T_nom,
                                                              parameters.pointsBackThroughParams.gap_Squre_B_nom)


        assemblyChords = AllCalculationChordsOfBladesParameter(delta_chord_T_B_array[:, 0], delta_chord_T_B_array[:, 1])

        return assemblyChords
